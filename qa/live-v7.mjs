...
for (let i = 0; i < 8; i++) { await page.mouse.move(ap.x + Math.cos(i*0.9)*42, ap.y + 30 + Math.sin(i*0.9)*30); await page.waitForTimeout(60); }
const hover = await page.evaluate(`window.__iglooInfo()`);
await page.screenshot({ path: 'qa/shots/live-v7-igloo.png' });
