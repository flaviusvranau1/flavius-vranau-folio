...
await page.waitForFunction(`window.__iglooRenderOnce !== undefined`, null, { timeout: 45000 });
await page.waitForTimeout(2500);
const iglooMs = await page.evaluate(`(() => { const t0 = performance.now(); for (let i=0;i<30;i++) window.__iglooRenderOnce(); return +((performance.now()-t0)/30).toFixed(2); })()`);
console.log({ nameMsPerFrame: nameMs, iglooMsPerFrame: iglooMs, budget60fps: 16.7, iglooBudget: 8 });
await browser.close();
